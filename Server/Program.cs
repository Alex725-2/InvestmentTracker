...
{
    // Ћокально используем SQL Server (LocalDB)
    builder.Services.AddDbContext<ApplicationDbContext>(options =>
        options.UseSqlServer(connectionString));
}
else
{
    // ¬ продакшене (VPS) используем SQLite
    builder.Services.AddDbContext<ApplicationDbContext>(options =>
...
// Hangfire Ц только локально, в продакшене отключаем (несовместим с SQLite)
if (builder.Environment.IsDevelopment())
{
...
// Hangfire Dashboard и повтор€ющиес€ задачи Ц только в Dev
if (app.Environment.IsDevelopment())
{
